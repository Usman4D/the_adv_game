...
pub fn calc_noise() {
    ...
    // let fbm_simplex = Fbm::<Simplex>::new(2);
    //
    // PlaneMapBuilder::<_, 2>::new(&fbm_simplex)
    //     .set_size(100, 100)
    //     .set_x_bounds(-5.0, 5.0)
    //     .set_y_bounds(-5.0, 5.0)
    //     .build()
    //     .write_to_file("assets/fbm.png");
    //
    wood_texture();
}
fn trait_objects_test() -> Box<dyn NoiseFn<f64, 2>> {
    let vein = Vein::new(128, 4.0);
    return Box::new(vein);
}
fn wood_texture() {
    ...
    // let combined_wood = Add::new(vein, wood_grain);
    //
    ...
    let test_trait = trait_objects_test();
    ...
    let planar_texture = PlaneMapBuilder::<_, 2>::new(terrain_vein_combined)
        .set_size(1024, 1024)
        .set_x_bounds(-160.0, 160.0)
        .set_y_bounds(-160.0, 160.0)
        .build();
